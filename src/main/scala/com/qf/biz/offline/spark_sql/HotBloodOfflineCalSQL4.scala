...
/**
  * Description：离线统计（新增用户，活跃用户，次日留存率~七日留存率）→ Spark SQL优化版, sql合并版(新增用户，活跃用户，次日留存率)<br/>
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      s"""
        ...
        |    max(stayCnt)*1.0/max(newUser) `次日留存率`
        | from
        | (
        |   (select
        ...
        |    0  `newUser`,
        |    0 `activeUser`,
        |     count(distinct t1.userName)   `stayCnt`
        |   from
