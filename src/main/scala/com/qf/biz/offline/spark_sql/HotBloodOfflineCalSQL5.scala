...
/**
  * Description：离线统计（新增用户，活跃用户，次日留存率~七日留存率）→ Spark SQL优化版, sql合并版(新增用户，活跃用户，次日留存率~7日留存率)<br/>
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      s"""
         ...
         |    max(nextDaystayCnt)*1.0/max(newUser) `次日留存率`,
         |    max(twoDaystayCnt)*1.0/max(newUser) `2日留存率`,
         |    max(threeDaystayCnt)*1.0/max(newUser) `3日留存率`,
         |    max(fourDaystayCnt)*1.0/max(newUser) `4日留存率`,
         |    max(fiveDaystayCnt)*1.0/max(newUser) `5日留存率`,
         |    max(sixDaystayCnt)*1.0/max(newUser) `6日留存率`,
         |    max(sevenDaystayCnt)*1.0/max(newUser) `7日留存率`
         | from
         | (
         |   (select
