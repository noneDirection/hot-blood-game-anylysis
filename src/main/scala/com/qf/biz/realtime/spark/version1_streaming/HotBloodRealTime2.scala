...
import com.qf.utils.{CommonUtil, JedisUtils}
...
/**
  * Description：实时统计（根据实时产生的游戏日志信息，追踪出那些开了外挂的游戏玩家）,使用广播变量进行优化<br/>
...
object HotBloodRealTime2 {
  def main(args: Array[String]): Unit = {
    ...
    val fdf: FastDateFormat = FastDateFormat.getInstance(CommonUtil.getPropertiesValueByKey(CommonData.TIME_PATTERN))
    ...
    val filterAndStandardDS: DStream[(String, Long)] = dsFromKafka.filter(perMsg => {
      ...
      (userName, bcFdf.value.parse(time).getTime)
    })
